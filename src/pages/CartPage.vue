  ...
  <div class="cart-page">
    ...
    <div v-if="cart.items && cart.items.length > 0" class="cart-container">
      ...
      <div class="cart-items">
        <div v-for="item in cart.items" :key="item.cartItemId" class="cart-item row">
          <div class="col-1">
            <input 
              type="checkbox" 
          ...
          <div class="col-2 price">¥{{ item.price.toFixed(2) }}</div>
  ...
  methods: {
    ...
    toggleSelectAll(event) {
      if (event.target.checked) {
        this.selectedItems = this.cart.items.map(item => item.cartItemId);
      } else {
        this.selectedItems = [];
      }
      this.calculateSelectedTotal();
    ...
    calculateSelectedTotal() {
      ...
      for (const item of this.cart.items) {
        if (this.selectedItems.includes(item.cartItemId)) {
          this.selectedTotal += item.price * item.quantity;
        }
      }
    },
