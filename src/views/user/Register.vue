...
import {storeListGet} from "../../api/store.ts"
...
onMounted(() => {
  storeListGet().then(res => {
    if (res.data.code === '000') {
      storeList.value = res.data.result;
      // 如果只有一个商店，也可以默认设置 storeId 为该商店的 id
      if (storeList.value.length === 1) {
        storeId.value = storeList.value[0].id;
      }
    } else {
      ElMessage({
        message: "获取商店数据失败: " + res.data.msg,
        type: 'error',
        center: true,
      });
    }
